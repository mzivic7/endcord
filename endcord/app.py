...
APP_COMMAND_AUTOCOMPLETE_DELAY = (
    0.3  # delay for requesting app command autocompleions after stop typing
)
MB = 1024 * 1024
USER_UPLOAD_LIMITS = (
    10 * MB,
    ...
    500 * MB,
    50 * MB,
)  # premium tier 0, 1, 2, 3 (none, classic, full, basic)
...
class Endcord:
    ...
    def __init__(self, screen, config, keybindings):
        ...
        if config["custom_user_agent"]:
            client_prop = client_properties.add_user_agent(
                client_prop, config["custom_user_agent"]
            )
        ...
        self.colors_formatted = self.tui.init_colors_formatted(
            self.colors_formatted, self.default_msg_alt_color
        )
    ...
    def switch_channel(
        ...
        logger.debug(
            f"Switching channel, has_id: {bool(channel_id)}, has_guild: {bool(guild_id)}, has hint: {bool(parent_hint)}"
        )
        ...
        if not self.forum and self.messages:
            self.add_to_channel_cache(
                self.active_channel["channel_id"],
                self.messages,
                self.active_channel.get("pinned", False),
            )
        ...
        # fetch messages or load them from cache
            ...
            if self.limit_channel_cache:
                for num, channel in enumerate(self.channel_cache):
                    if channel[0] == channel_id and not (
                        len(channel) > 3 and channel[3]
                    ):
                        from_cache = True
                        break
        ...
        elif self.current_channel.get("type") in (11, 12) and self.current_channel.get(
            "locked"
        ):
        ...
        self.gateway.set_subscribed_channels(
            [x[0] for x in self.channel_cache] + [channel_id]
        )
        if not forum:
        ...
        if self.config["remember_state"] and self.current_channel.get("type") not in (
            11,
            12,
            15,
        ):
                        ...
                        if "primary_role_color" not in member:
                            ...
                            for role in self.current_roles:
                                if role["id"] in member_roles:
                                    ...
                                    member["primary_role_alt_color"] = role.get(
                                        "alt_color_id"
                                    )
                                    break
        ...
        if self.limit_channel_cache:
            ...
            for num, channel in enumerate(self.channel_cache):
                if channel[0] == channel_id:
                    self.channel_cache[num] = [
                        channel_id,
                        messages[: self.msg_num],
                        set_pinned,
                    ]
                    break
            else:
                self.channel_cache.append(
                    [channel_id, messages[: self.msg_num], set_pinned]
                )
        ...
        if not self.forum:
            if self.active_channel.get("pinned"):
                ...
                if pinned >= self.limit_channel_cache:  # if all are pinned
                    self.update_extra_line(
                        "Can't add tab: channel cache limit reached."
                    )
                else:
    ...
    def wait_input(self):
        ...
        while self.run:
            if self.restore_input_text[1] == "prompt":
                ...
                input_text, chat_sel, tree_sel, action = self.tui.wait_input(
                    self.prompt, forum=self.forum
                )
            elif self.restore_input_text[1] in (
                "standard",
                "standard extra",
                "standard insert",
            ):
                keep_cursor = True
                if self.restore_input_text[
                    1
                ] == "standard" and not self.restore_input_text[0].startswith("/"):
                    self.stop_extra_window()
                ...
                if restore_text:
                    ...
                    input_text, chat_sel, tree_sel, action = self.tui.wait_input(
                        self.prompt, clear_delta=True, forum=self.forum
                    )
            ...
            # set delete
                ...
                msg_index = self.lines_to_msg(chat_sel)
                if (
                    self.messages[msg_index]["user_id"] == self.my_id
                    or self.active_channel["admin"]
                ):
                    if "deleted" not in self.messages[msg_index]:
                        self.reset_actions()
            ...
            # toggle mention ping
                ...
                self.restore_input_text = (input_text, "standard")
                self.replying["mention"] = (
                    None
                    if self.replying["mention"] is None
                    else not self.replying["mention"]
                )
                self.update_status_line()
                ...
                if len(selected_urls) == 1:
                    ...
                    self.download_threads.append(
                        threading.Thread(
                            target=self.download_file,
                            daemon=True,
                            args=(selected_urls[0], False, True),
                        )
                    )
                    self.download_threads[-1].start()
                else:
            ...
            # join/leave selected thread in tree
                ...
                self.restore_input_text = (input_text, "standard")
                if self.tree_metadata[tree_sel] and self.tree_metadata[tree_sel][
                    "type"
                ] in (11, 12):
                ...
                if self.uploading:
                    ...
                    if isinstance(file_path, str):
                        self.media_thread = threading.Thread(
                            target=self.open_media, daemon=True, args=(file_path,)
                        )
            ...
            # view profile info
                ...
                if (
                    self.viewing_user_data["id"] != user_id
                    or self.viewing_user_data["guild_id"] != guild_id
                ):
                    if guild_id:
                        self.viewing_user_data = self.discord.get_user_guild(
                            user_id, guild_id
                        )
                    else:
            ...
            # select in extra window / memeber list
                ...
                if self.extra_window_open and action == 27:
                    if self.extra_indexes:
                        ...
                        if (
                            channel_id
                            and channel_id != self.active_channel["channel_id"]
                        ):
                            guild_id = self.active_channel["guild_id"]
                ...
                ):  # controls for memeber list when no extra window
            ...
            # search
                ...
                if not self.search:
                    ...
                    extra_title, extra_body = formatter.generate_extra_window_text(
                        "Search:", SEARCH_HELP_TEXT, max_w
                    )
                    self.stop_assist(close=False)
            ...
            # copy channel link
                ...
                if guild_id and self.tree_metadata[tree_sel]:
                    ...
                    url = (
                        f"https://{self.discord.host}/channels/{guild_id}/{channel_id}"
                    )
            ...
            # go to channel/message mentioned in this message
                ...
                for match in re.finditer(
                    formatter.match_discord_channel_combined,
                    self.messages[msg_index]["content"],
                ):
                ...
                if not channels:
                    continue
                if len(channels) == 1:
                    channel_id, channel_name, guild_id, guild_name, parent_hint = (
                        self.find_parents_from_id(channels[0][1])
                    )
                    if channel_name:
                        self.switch_channel(
                            channel_id,
            ...
            # command
                ...
                if not self.command:
                    ...
                    self.assist_type = 5
                    self.tui.instant_assist = True
                    self.tui.draw_extra_window(
                        extra_title, extra_body, select=True, start_zero=True
                    )
                    self.extra_window_open = True
                    self.extra_bkp = (
                        self.tui.extra_window_title,
                        self.tui.extra_window_body,
                    )
                else:
            ...
            # mouse double-click on message
                ...
                if clicked_chat is not None and mouse_x is not None:
                    ...
                    if chat_line_map:
                        ...
                        if (
                            chat_line_map[1]
                            and chat_line_map[1][0] < mouse_x < chat_line_map[1][1]
                        ):
                            ...
                            chat_line_map[4]
                            and chat_line_map[4][0] < mouse_x < chat_line_map[4][1]
                        ):
                        ...
                        if (
                            clicked_type == 1
                            and "deleted" not in self.messages[msg_index]
                        ):  # start reply
                            if self.messages[msg_index]["user_id"] == self.my_id:
                                mention = None
                            else:
                            ...
                            if (
                                ...
                                if guild_id:
                                    self.viewing_user_data = (
                                        self.discord.get_user_guild(user_id, guild_id)
                                    )
                                else:
                                    self.viewing_user_data = self.discord.get_user(
                                        user_id
                                    )
                            self.stop_assist(close=False)
                            self.view_profile(self.viewing_user_data)
                            ...
                            clicked_type == 4 and selected is not None
                        ):  # add/remove reaction
                            ...
                            url = urls[url_index]
                            embed_url = False
                            for embed in self.get_msg_embeds(
                                msg_index, media_only=False
                            ):
                                if embed == url:
                                    embed_url = True
                                    break
                            ...
                            if match:
                                if shutil.which(self.config["yt_dlp_path"]) and (
                                    (
                                        support_media
                                        and not self.config["native_media_player"]
                                    )
                                    or shutil.which(self.config["mpv_path"])
                                ):
                                    self.download_threads.append(
                                        threading.Thread(
                                            target=self.download_file,
            ...
            # enter
                ...
                if input_text.lower() != "y" and (
                    self.deleting
                    or self.cancel_download
                    or self.hiding_ch["channel_id"]
                ):
                ...
                if self.editing:
                    text_to_send = emoji.emojize(
                        input_text, language="alias", variant="emoji_type"
                    )
                    ...
                    if num <= len(self.going_to_ch):
                        ...
                        channel_id, channel_name, guild_id, guild_name, parent_hint = (
                            self.find_parents_from_id(channel_id)
                        )
                        if channel_name:
                            self.switch_channel(
                                channel_id,
                    ...
                    try:
                        ...
                        if num <= len(reactions):
                            ...
                            max_w = self.tui.get_dimensions()[2][1]
                            extra_title, extra_body = (
                                formatter.generate_extra_window_reactions(
                                    reactions[num], reaction_details, max_w
                                )
                            )
                            self.tui.draw_extra_window(extra_title, extra_body)
                            self.extra_window_open = True
                    ...
                    input_text[0] == "/"
                    and parser.check_start_command(
                        input_text,
                        self.my_commands,
                        self.guild_commands,
                        self.guild_commands_permitted,
                    ...
                    for num, attachments in enumerate(self.ready_attachments):
                        if (
                            ...
                            this_attachments = self.ready_attachments.pop(num)[
                                "attachments"
                            ]
                            self.update_extra_line()
                            break
                    # if this thread is not joined, join it (locally only)
                    if self.current_channel.get("type") in (
                        11,
                        12,
                    ) and not self.current_channel.get("joined"):
                        channel_id, _, guild_id, _, parent_id = self.find_parents(
                            self.active_channel["channel_id"]
                        )
                        self.thread_togle_join(
                            guild_id,
                    ...
                    for match in re.finditer(formatter.match_sticker_id, input_text):
                        ...
                        input_text = (
                            input_text[: match.start()] + input_text[match.end() :]
                        )
                    text_to_send = emoji.emojize(
                        input_text, language="alias", variant="emoji_type"
                    )
                    if self.fun and (
                        "xyzzy" in text_to_send or "XYZZY" in text_to_send
                    ):
    ...
    def execute_command(self, cmd_type, cmd_args, chat_sel, tree_sel):
        ...
        if cmd_type == 0:
            ...
            if len(selected_urls) == 1 or select_num:
                ...
                self.download_threads.append(
                    threading.Thread(
                        target=self.download_file, daemon=True, args=(selected_url,)
                    )
                )
                self.download_threads[-1].start()
            else:
            ...
            if self.current_channel.get("allow_attach", True):
                ...
                if path:
                    self.upload_threads.append(
                        threading.Thread(target=self.upload, daemon=True, args=(path,))
                    )
            ...
            cmd_type == 11
            and self.tree_metadata[tree_sel]
            and self.tree_metadata[tree_sel]["type"] in (11, 12)
        ):  # TOGGLE_THREAD
            thread_id = self.tree_metadata[tree_sel]["id"]
            guild_id, channel_id, _ = self.find_parents(tree_sel)
            self.thread_togle_join(guild_id, channel_id, thread_id)
            ...
            if len(channels) == 1 or select_num is not None:
                ...
                channel_id, channel_name, guild_id, guild_name, parent_hint = (
                    self.find_parents_from_id(channels[select_num][1])
                )
                if channel_name:
                    self.switch_channel(
                        channel_id,
            ...
            channel_id, channel_name, guild_id, guild_name, parent_hint = (
                self.find_parents_from_id(object_id)
            )
            # guild
            if not channel_id:
            ...
            if not avatar_id:
                for dm in self.dms:
                    if dm["id"] == user_id:
                        ...
                        break
            if not avatar_id:
                avatar_id = self.discord.get_user(user_id, extra=True)["extra"][
                    "avatar"
                ]
            if avatar_id:
                ...
                if pfp_path:
                    self.media_thread = threading.Thread(
                        target=self.open_media, daemon=True, args=(pfp_path,)
                    )
                    self.media_thread.start()
            ...
            for num, channel in enumerate(self.input_store):
                if channel["id"] == self.active_channel["channel_id"]:
                    ...
                    self.input_store[num]["content"] = (
                        input_text[:input_index] + timestamp + input_text[input_index:]
                    )
                    self.input_store[num]["index"] = len(
                        input_text[:input_index] + timestamp
                    )
                    break
            ...
            if poll:
                if poll["expires"] > time.time():
                    if select_num > 0 and select_num <= len(poll["options"]):
                        ...
                        self.update_extra_line(
                            "Can't vote - selected answer doesn't exist."
                        )
                else:
            ...
            msg_index = self.lines_to_msg(chat_sel)
            if not self.active_channel["guild_id"] or (
                self.active_channel["admin"] or self.active_channel.get("allow_manage")
            ):
            ...
            if "component_info" in message and message["component_info"]["buttons"]:
                ...
                if "num" in cmd_args:
                    try:
                        button = message["component_info"]["buttons"][
                            int(cmd_args["num"]) - 1
                        ]
                        custom_id = button["id"]
            ...
            if self.forum:
                ...
                self.update_extra_line(
                    f"Chat saved to: {os.path.join(peripherals.log_path, 'Debug')}"
                )
            ...
            if guild_id:  # set channel/category
                if cmd_args["setting"].startswith("suppress"):
                    ...
                    self.discord.send_notification_setting_channel(
                        cmd_args["setting"], channel_id, guild_id
                    )
            else:
                for dm in self.dms:
                    if dm["id"] == channel_id:
                    ...
                    if guild:
                        if cmd_args["setting"] == "suppress_everyone":
                            ...
                            value = None
                        self.discord.send_notification_setting_guild(
                            cmd_args["setting"], channel_id, value
                        )
                    else:
            ...
            if search_text:
                ...
                self.remove_running_task("Searching gifs", 4)
                extra_title, extra_body = formatter.generate_extra_window_search_gif(
                    self.search_messages, max_w
                )
                self.tui.draw_extra_window(extra_title, extra_body, select=True)
                self.restore_input_text = (None, "search")
    ...
    def execute_app_command(self, input_text, autocomplete=False):
        ...
        if logger.getEffectiveLevel() == logging.DEBUG:
            ...
            debug.save_json(
                self.guild_commands_permitted, "commands_guild_permitted.json"
            )
    ...
    def go_bottom(self):
        ...
        if self.messages[0]["id"] != self.last_message_id:
            ...
            if self.limit_channel_cache:
                for num, channel in enumerate(self.channel_cache):
                    if channel[0] == self.active_channel["channel_id"] and not (
                        len(channel) > 3 and channel[3]
                    ):
                        from_cache = True
                        break
            ...
            # downlaod messages
                ...
                self.messages = self.get_messages_with_members()
                self.update_chat()
                self.tui.allow_chat_selected_hide(
                    self.messages[0]["id"] == self.last_message_id
                )
    ...
    def switch_tab(self, select_num):
        ...
        if channel_id:
            ...
            self.switch_channel(
                channel_id, channel_name, guild_id, guild_name, parent_hint=parent_hint
            )
        ...
        for embed in message["embeds"]:
            if embed["url"] and not embed.get("hidden") and embed["url"] not in content:
                if content:
                    content += "\n"
                content += (
                    f"[{formatter.clean_type(embed['type'])} embed]:\n{embed['url']}"
                )
    ...
    def download_file(self, url, move=True, open_media=False, open_move=False):
        ...
        if match:
            ...
            if open_media:
                ...
                self.media_thread = threading.Thread(
                    target=self.open_media, daemon=True, args=(url,)
                )
                self.media_thread.start()
        ...
        if not open_media or not destination:
                ...
                if path:
                    ...
                    if move:
                        if not os.path.exists(self.downloads_path):
                            os.makedirs(
                                os.path.dirname(self.downloads_path), exist_ok=True
                            )
                        destination = os.path.join(
                            self.downloads_path, os.path.basename(path)
                        )
            ...
            if move:
                self.update_extra_line(
                    f"File saved to {peripherals.collapseuser(self.downloads_path)}"
                )

        # open media
        if open_media:
            self.media_thread = threading.Thread(
                target=self.open_media, daemon=True, args=(destination,)
            )
            self.media_thread.start()
    ...
    def upload(self, path):
        ...
        if size > limit:
            self.update_extra_line(
                f"File is larger than current upload limit: {int(limit / MB)}MB"
            )
            return
        if size > 200 * MB:
            self.update_extra_line(
                "Cant upload over cloudflare. File is larger than 200MB."
            )
            return
        ...
        self.add_running_task("Uploading file", 2)
        self.update_extra_line()
        upload_data, code = self.discord.request_attachment_link(
            self.active_channel["channel_id"], path
        )
        try:
            if upload_data:
                uploaded = self.discord.upload_attachment(
                    upload_data["upload_url"], path
                )
                if uploaded:
                    self.ready_attachments[ch_index]["attachments"][at_index][
                        "upload_url"
    ...
    def cancel_attachment(self):
        ...
        for num, attachments_ch in enumerate(self.ready_attachments):
            if attachments_ch["channel_id"] == self.active_channel["channel_id"]:
                ...
                if attachments:
                    attachment = attachments.pop(self.selected_attachment)[
                        "upload_filename"
                    ]
    ...
    def get_chat_chunk(self, past=True):
        ...
        if past:
            ...
            if (
                new_chunk
                and self.messages[0]["id"] == self.last_message_id
                and new_chunk[0]["id"] != self.last_message_id
            ):
                self.add_to_channel_cache(
                    self.active_channel["channel_id"],
                    self.messages,
    ...
    def go_to_message(self, message_id):
        ...
        for num, message in enumerate(self.messages):
            ...
            if new_messages:
                if (
                    self.messages[0]["id"] == self.last_message_id
                    and new_messages[0]["id"] != self.last_message_id
                ):
                    self.add_to_channel_cache(
                        self.active_channel["channel_id"],
                        self.messages,
    ...
    def get_forum_chunk(self, force=False):
        ...
        if self.forum or force:
            ...
            if num_threads:
                self.update_forum(
                    self.active_channel["guild_id"], self.active_channel["channel_id"]
                )
                self.tui.update_chat(self.chat, self.chat_format)
            else:
                self.chat_end = True
        ...
        if user_id == self.my_id:
            ...
            for presence in self.activities:
                if "id" in presence and presence["id"] == user_id:
                    selected_presence = presence
                    break
        extra_title, extra_body = formatter.generate_extra_window_profile(
            user_data, roles, selected_presence, max_w
        )
        self.tui.draw_extra_window(extra_title, extra_body)
        self.extra_window_open = True
    ...
    def view_channel(self, channel, guild=False):
        ...
        max_w = self.tui.get_dimensions()[2][1]
        if guild:
            extra_title, extra_body = formatter.generate_extra_window_guild(
                channel, max_w
            )
        else:
            extra_title, extra_body = formatter.generate_extra_window_channel(
                channel, max_w
            )
        self.tui.draw_extra_window(extra_title, extra_body)
        self.extra_window_open = True
    ...
    def view_summaries(self, channel_id=None):
        ...
        self.stop_assist(close=False)
        max_w = self.tui.get_dimensions()[2][1]
        extra_title, extra_body, self.extra_indexes = (
            formatter.generate_extra_window_summaries(summaries, max_w)
        )
        self.tui.draw_extra_window(extra_title, extra_body, select=True)
        self.extra_window_open = True
    ...
    def do_view_reactions(self, msg_index):
        ...
        if reactions:
            if len(self.messages[msg_index]["reactions"]) == 1:
                ...
                self.stop_assist(close=False)
                max_w = self.tui.get_dimensions()[2][1]
                extra_title, extra_body = formatter.generate_extra_window_reactions(
                    reactions[0], reaction_details, max_w
                )
                self.tui.draw_extra_window(extra_title, extra_body)
                self.extra_window_open = True
            else:
    ...
    def do_search(self, text):
        ...
        content, channel_id, author_id, mentions, has, max_id, min_id, pinned = (
            parser.search_string(text)
        )
        self.search = (
            content,
            channel_id,
        ...
        extra_title, self.extra_body, self.extra_indexes = (
            formatter.generate_extra_window_search(
                self.search_messages,
                self.current_roles,
                self.current_channels,
    ...
    def extend_search(self):
        ...
        if search_chunk:
            ...
            if len(self.search_messages) >= total_search_messages:
                self.search_end = True
            extra_title, extra_body_chunk, indexes_chunk = (
                formatter.generate_extra_window_search(
                    search_chunk,
                    self.current_roles,
                    self.current_channels,
            ...
            self.tui.draw_extra_window(
                extra_title, self.extra_body, select=len(self.extra_body)
            )
        self.remove_running_task("Searching", 4)
    ...
    def assist(self, assist_word, assist_type, query_results=None):
        ...
        if assist_type == 1:  # channels
            ...
            if assist_word.lower().startswith("set "):
                ...
                if channel_id:
                    ...
                    channel_id = self.tree_metadata[tree_sel]["id"]
                    guild_id = self.find_parents(tree_sel)[0]
                if assist_word.endswith(" ") and not all(
                    not x for x in query_words[1:]
                ):
            ...
            if autocomplete:
                ...
                if (
                    self.allow_app_command_autocomplete
                    and self.app_command_autocomplete != query
                    and time.time() - self.app_command_sent_time
        ...
        max_w = self.tui.get_dimensions()[2][1]
        extra_title, extra_body = formatter.generate_extra_window_assist(
            self.assist_found, assist_type, max_w
        )
        self.extra_window_open = True
        if (self.search or self.search_gif or self.command) and not (
            self.assist_word or self.assist_word == " "
        ):
    ...
    def stop_assist(self, close=True):
        ...
        if self.assist_word:
            ...
            if (self.search or self.search_gif or self.command) and self.extra_bkp:
                self.extra_window_open = True
                self.tui.draw_extra_window(
                    self.extra_bkp[0], self.extra_bkp[1], select=True
                )
    ...
    def insert_assist(self, input_text, index, start, end):
        ...
        if self.assist_type == 1:  # channel
            ...
            if match:
                # replace word after "="
                if match.group(2):
                    new_text = input_text[: -len(match.group(2))] + str(
                        self.assist_found[index][1]
                    )
                else:
            ...
            if (
                self.assist_found[index][1] is True
                or self.assist_found[index][1] == input_text
            ):
                if self.uploading:
                    self.upload_threads.append(
                        threading.Thread(
                            target=self.upload, daemon=True, args=(input_text,)
                        )
                    )
                    self.upload_threads[-1].start()
                self.uploading = False
    ...
    def open_media(self, path):
        ...
        if support_media and not self.config["native_media_player"]:
            ...
            if shutil.which(self.config["yt_dlp_path"]) and shutil.which(
    ...
    def update_chat(self, keep_selected=True, change_amount=0):
        ...
        if keep_selected:
            ...
            self.unseen_scrolled = False
            self.update_status_line()
        self.chat, self.chat_format, self.chat_indexes, self.chat_map = (
            formatter.generate_chat(
                self.messages,
                self.current_roles,
                self.current_channels,
                self.chat_dim[1],
    ...
    def update_member_list(self, last_index=None, reset=False):
        ...
        if (
            ...
            and not self.tui.mlist_index - 1
            < last_index
            < self.tui.mlist_index - 1 + self.screen.getmaxyx()[0]
        ):
    ...
    def update_tabs(self, no_redraw=False, add_current=False):
        ...
        if (
            add_current
            and self.active_channel["pinned"]
            and active_channel_id not in tabs
        ):
    ...
    def update_status_line(self):
        ...
        self.tui.update_status_line(
            status_line_l, status_line_r, status_line_l_format, status_line_r_format
        )
        ...
        if self.format_title_line_l:
            ...
            self.tui.update_title_line(
                title_line_l, title_line_r, title_line_l_format, title_line_r_format
            )
        ...
        if custom_text:
            ...
            if attachments:
                ...
                self.extra_line = formatter.generate_extra_line(
                    attachments["attachments"], self.selected_attachment, statusline_w
                )
                self.tui.draw_extra_line(self.extra_line)
            else:
                self.extra_line = None
    ...
    def set_seen(self, target_id, force=False, ack=True):
        ...
        for unseen_channel in self.unseen:
            if unseen_channel["channel_id"] == target_id:  # find this unseen chanel
                ...
                if (
                    not force
                ):  # dont remove notification when marking seen current channel
        ...
        if to_sort:
            for guild in self.threads:
                if guild["guild_id"] == guild_id or guild_id is None:
                    for channel in guild["channels"]:
                        channel["threads"] = sorted(
                            channel["threads"], key=lambda x: x["id"], reverse=True
                        )
                break
        self.update_tree()
        if self.forum:
        ...
        if new_tree_format:
            ...
            for num, code in enumerate(self.tree_format):
                if code < 300 and (code % 10) == 0:  # guild/category
                    ...
                    499 < code < 700 and (code % 10) == 1
                ):  # channel (with threads) and forum
                ...
                if data["id"] == loaded_message["id"]:
                    if op == "MESSAGE_UPDATE":
                        ...
                        for num2, reaction in enumerate(loaded_message["reactions"]):
                            if (
                                data["emoji_id"] == reaction["emoji_id"]
                                and data["emoji"] == reaction["emoji"]
                            ):
                        ...
                        op in ("MESSAGE_POLL_VOTE_ADD", "MESSAGE_POLL_VOTE_REMOVE")
                        and "poll" in loaded_message
                    ):
                        if "poll" in loaded_message:
                            add = op == "MESSAGE_POLL_VOTE_ADD"
                            for num2, option in enumerate(
                                loaded_message["poll"]["options"]
                            ):
                                if option["id"] == data["answer_id"]:
                                    loaded_message["poll"]["options"][num2][
                                        "count"
                                    ] += 1 if add else -1
                                    if data["user_id"] == self.my_id:
                                        loaded_message["poll"]["options"][num2][
                    ...
                    if op == "MESSAGE_UPDATE":
                        ...
                        if "poll" in loaded_message:
                            ...
                            for num2, option in enumerate(
                                ...
                                if option["id"] == data["answer_id"]:
                                    loaded_message["poll"]["options"][num2]["count"] + (
                                        1 if add else -1
                                    )
                                    if data["user_id"] == self.my_id:
                                        loaded_message["poll"]["options"][num2][
    ...
    def process_msg_events_other_channels(self, new_message, this_channel):
        ...
        op = new_message["op"]
        skip_unread = (
            data["channel_id"] == self.active_channel["channel_id"]
            and not self.unseen_scrolled
        )
        ...
        if op == "MESSAGE_CREATE" and data["user_id"] != self.my_id:
            ...
            for guild in self.guilds:
                if guild["guild_id"] == data["guild_id"]:
                    if guild.get("muted"):
                        break
                    for channel in guild["channels"]:
                        if new_message_channel_id == channel["id"] and (
                            channel.get("muted") or channel.get("hidden")
                        ):
                            muted = True
                            break
                    break
            if not muted:
                ...
                if (
                    ...
                    or bool([i for i in my_roles if i in data["mention_roles"]])
                    or self.my_id in [x["id"] for x in mentions]
                    or new_message_channel_id in self.dms_vis_id
                ):
                    if not skip_unread:
                        ping = True
    ...
    def process_msg_events_ghost_ping(self, new_message):
        ...
        if new_message["op"] == "MESSAGE_DELETE" and not self.keep_deleted:
            ...
            for num, pinged_channel in enumerate(self.unseen):
                if (
                    new_message_channel_id == pinged_channel["channel_id"]
                    and new_message["d"]["id"]
                    in pinged_channel[
                        "mentions"
                    ...
                    if self.enable_notifications:
                        for num_1, notification in enumerate(self.notifications):
                            if notification["channel_id"] == new_message_channel_id:
                                notification_id = self.notifications.pop(num_1)[
                                    "notification_id"
                                ]
                                peripherals.notify_remove(notification_id)
                                break
    ...
    def update_presence_from_proto(self):
        ...
        if "status" in self.discord_settings:
            ...
            if "customStatus" in self.discord_settings["status"]:
                custom_status_emoji = {
                    "id": self.discord_settings["status"]["customStatus"].get(
                        "emojiID"
                    ),
                    "name": self.discord_settings["status"]["customStatus"].get(
                        "emojiName"
                    ),
                    "animated": self.discord_settings["status"]["customStatus"].get(
                        "animated", False
                    ),
                }
                custom_status = self.discord_settings["status"]["customStatus"]["text"]
            if custom_status_emoji and not (
                custom_status_emoji["name"] or custom_status_emoji["id"]
            ):
                custom_status_emoji = None
        else:  # just in case
            status = "online"
    ...
    def main(self):
        ...
        if "guildFolders" in self.discord_settings:
            ...
            for guild in self.discord_settings["guildFolders"].get(
                "guildPositions", []
            ):
        ...
        if logger.getEffectiveLevel() == logging.DEBUG:
            debug.save_json(
                debug.anonymize_guild_positions(self.guild_positions),
                "guild_positions.json",
            )
        ...
        self.tui.update_chat(
            ["Loading channels", "Connecting to Discord"], [[[self.colors[0]]]] * 2
        )
        ...
        if support_media:
            ...
            self.curses_media = media.CursesMedia(
                self.screen, self.config, last_free_color_id
            )
        else:
        ...
        if self.state["last_channel_id"]:
            self.tui.update_chat(
                ["Loading messages", "Loading channels", "Connecting to Discord"],
                [[[self.colors[0]]]] * 3,
            )
            ...
            if channel_name:
                self.switch_channel(
                    ...
                    guild_id,
                    guild_name,
                    open_member_list=self.member_list_auto_open,
                )
                self.tui.tree_select_active()
        else:
            self.tui.update_chat(
                [
                    "Select channel to load messages",
                    "Loading channels",
        ...
        if (
            self.member_list_auto_open
            and self.active_channel["guild_id"]
            and self.screen.getmaxyx()[1]
            - self.config["tree_width"]
            - self.member_list_width
            - 2
            >= 32
        ...
        # start input thread
        self.wait_input_thread = threading.Thread(
            target=self.wait_input, daemon=True, args=()
        )
        ...
        while self.run:
            ...
            while self.run:
                ...
                if new_message:
                    new_message_channel_id = new_message["d"]["channel_id"]
                    this_channel = (
                        new_message_channel_id == self.active_channel["channel_id"]
                    )
                    if this_channel:
                        self.process_msg_events_active_channel(
                            new_message, selected_line
                        )
                    ...
                    if not this_channel or (
                        this_channel
                        and (self.unseen_scrolled or self.ping_this_channel)
                    ):
                        self.process_msg_events_other_channels(
                            new_message, this_channel
                        )
                    # remove ghost pings
            ...
            while self.run:
                ...
                if new_typing:
                    if (
                        new_typing["channel_id"] == self.active_channel["channel_id"]
                        and new_typing["user_id"] not in self.blocked
                        and new_typing["user_id"] != self.my_id
                    ):
                        if not new_typing["username"]:  # its DM
                            for dm in self.dms:
                                if dm["id"] == new_typing["channel_id"]:
                                    new_typing["username"] = dm["recipients"][0][
                                        "username"
                                    ]
                                    new_typing["global_name"] = dm["recipients"][0][
                                        "global_name"
                                    ]
                                    break
                        for num, user in enumerate(self.typing):
            ...
            while self.run:
                ...
                if new_message_ack:
                    dont_remove_notify = (
                        new_message_ack["channel_id"]
                        == self.active_channel["channel_id"]
                    )
                    self.set_seen(
                        new_message_ack["channel_id"],
                        ack=False,
                        force=dont_remove_notify,
                    )
                else:
                    break
            ...
            if self.send_my_typing and not self.disable_sending:
                ...
                if (
                    not self.ignore_typing
                    and my_typing
                    and time.time() >= self.typing_sent + 7
                ):
            ...
            # send pending ack
            if (
                not self.unseen_scrolled
                and self.pending_ack
                and time.time() - self.sent_ack_time > self.ack_throttling
            ):
                self.set_seen(self.active_channel["channel_id"], force=True)
                self.sent_ack_time = time.time()
                self.pending_ack = False
            ...
            if assist_type and not self.uploading:
                if (
                    assist_type == 100
                ):  # or (" " in assist_word and assist_type not in (5, 6)):
                    self.stop_assist()
                elif assist_type == 6:  # app commands
                    if assist_word != self.assist_word and not (
                        self.disable_sending or self.forum
                    ):
                        self.ignore_typing = True
                        if not self.got_commands:
                            # this will be allowed to run when channel changes
                            self.got_commands = True
                            self.my_commands, self.my_apps = (
                                self.discord.get_my_commands()
                            )
                            if self.active_channel["guild_id"]:
                                self.guild_commands, self.guild_apps = (
                                    self.discord.get_guild_commands(
                                        self.active_channel["guild_id"]
                                    )
                                )
                        ...
                        not self.allow_app_command_autocomplete
                        and time.time() - self.app_command_last_keypress
                        >= APP_COMMAND_AUTOCOMPLETE_DELAY
                    ):
                        self.allow_app_command_autocomplete = True
                    app_command_autocomplete_resp = (
                        self.gateway.get_app_command_autocomplete_resp()
                    )
            ...
            if self.assist_type == 2:
                ...
                if query_results:
                    self.assist(
                        self.assist_word, self.assist_type, query_results=query_results
                    )
