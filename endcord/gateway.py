...
class Gateway:
    ...
    def connect(self):
        ...
        self.heartbeat_interval = int(
            json.loads(zlib_decompress(self.ws.recv()))["d"]["heartbeat_interval"]
        )
        ...
        self.heartbeat_thread = threading.Thread(
            target=self.send_heartbeat, daemon=True, args=()
        )
    ...
    def add_member_roles(self, guild_id, user_id, roles):
        ...
        for member in self.member_roles[num]["members"]:
            if member["user_id"] == user_id:
                return
        self.member_roles[num]["members"].insert(
            0,
            {
                "user_id": user_id,
                "roles": roles,
        ...
        while self.run and not self.wait:
            ...
            if opcode == 11:
                ...
                if optext == "READY":
                    ...
                    self.premium = data["user"][
                        "premium_type"
                    ]  # 0 - none, 1 - classic, 2 - full, 3 - basic
                    # guilds and channels
                    if ("guilds" not in data) and ("user_guild_settings" in data):
                        logger.warn(
                            "Abnormal READY event received, if its always happening, report this"
                        )
                        self.resumable = True
                        break
                    abnormal = False
                    for guild in data["guilds"]:
                        ...
                        for channel in guild["channels"]:
                            ...
                            if (
                                "last_message_id" in channel and channel["type"] != 15
                            ):  # skip forums
                                last_messages.append(
                                    {
                                        "message_id": channel[
                                            "last_message_id"
                                        ],  # really last message id
                        ...
                        # sort roles
                        guild_roles = sorted(
                            guild_roles, key=lambda x: x.get("position"), reverse=True
                        )
                        guild_roles = sorted(
                            guild_roles, key=lambda x: not bool(x.get("color"))
                        )
                        self.roles.append(
                            {
                                "guild_id": guild_id,
                            ...
                            threads.append(
                                {
                                    ...
                                    "timestamp": thread["thread_metadata"][
                                        "create_timestamp"
                                    ],
                    ...
                    time_log_string += (
                        f"    guilds - {round(time.time() - ready_time_start, 3)}s\n"
                    )
                    ...
                    self.dms = sorted(
                        self.dms, key=lambda x: x["last_message_id"], reverse=True
                    )
                    ...
                    time_log_string += (
                        f"    DMs - {round(time.time() - ready_time_mid, 3)}s\n"
                    )
                    ready_time_mid = time.time()
                        ...
                        if "last_message_id" in channel and "mention_count" in channel:
                            if channel["mention_count"] != 0:
                                ...
                                for last_message in last_messages:
                                    if (
                                        channel["id"] == last_message["channel_id"]
                                        and channel["last_message_id"]
                        ...
                        self.msg_unseen.append(
                            {
                                ...
                                "mentions": ["True"]
                                if channel_id in msg_ping
                                else [],  # mesage_id is unknown
                            }
                        )
                    ...
                    for guild in data["user_guild_settings"]["entries"]:
                        if guild["guild_id"]:
                            ...
                            opt_in_channels = not perms.decode_flag(
                                guild_flags, 14
                            ) or perms.decode_flag(guild_flags, 13)
                            self.guilds[guild_num]["opt_in_channels"] = opt_in_channels
                            for channel in guild["channel_overrides"]:
                                found = False
                                for channel_num, channel_g in enumerate(
                                    self.guilds[guild_num]["channels"]
                                ):
                    ...
                    for guild_num, guild in enumerate(self.guilds):
                        ...
                        for category_num, category in enumerate(guild["channels"]):
                            if (
                                owned or not community or opt_in_channels
                            ):  # cant hide channels in owned and non-community guild
                                self.guilds[guild_num]["channels"][category_num][
                                    "hidden"
                                ] = False
                                continue
                            if category["type"] == 4:
                    ...
                    time_log_string += (
                        f"    protobuf - {round(time.time() - ready_time_mid, 3)}s\n"
                    )
                    ready_time_mid = time.time()
                    # get my roles
                    ...
                    time_log_string += (
                        f"    roles - {round(time.time() - ready_time_mid, 3)}s\n"
                    )
                    ready_time_mid = time.time()
                    # write debug data
                    if logger.getEffectiveLevel() == logging.DEBUG:
                        debug.save_json(
                            debug.anonymize_guilds(self.guilds), "guilds.json"
                        )
                    # blocked users
                    time_log_string += (
                        f"    debug data - {round(time.time() - ready_time_mid, 3)}s\n"
                    )
                    self.ready = True
                    time_log_string += (
                        f"    total - {round(time.time() - ready_time_start, 3)}s"
                    )
                    ...
                    if "guild_id" in data:
                        ...
                        for guild_activities in self.subscribed_activities:
                            ...
                            selected_activities = self.subscribed_activities[-1][
                                "members"
                            ]
                    ...
                    for thread in response["d"]["threads"]:
                        if not guild_id:
                            guild_id = thread[
                                "guild_id"
                            ]  # assuming its one event per thread
                        threads.append(
                            {
                                "id": thread["id"],
                                "type": thread["type"],
                                ...
                                "locked": thread["thread_metadata"]["locked"],
                                "message_count": thread["message_count"],
                                "timestamp": thread["thread_metadata"].get(
                            ...
                            "threads": [
                                {
                                    ...
                                    "timestamp": data["thread_metadata"][
                                        "create_timestamp"
                                    ],
                        ...
                        if memlist["op"] == "SYNC":
                            ...
                            for item in memlist["items"]:
                                if "group" in item:
                                    ...
                                    activities = []
                                    for activity in member_data["presence"][
                                        "activities"
                                    ]:
                                        if activity["type"] == 4:
                                            custom_status = activity.get("state", "")
                                            ...
                                            activities.append(
                                                {
                                                    ...
                                                    "state": activity.get("state"),
                                                    "details": activity.get("details"),
                                                    "small_text": assets.get(
                                                        "small_text"
                                                    ),
                                                    "large_text": assets.get(
                                                        "large_text"
                                                    ),
                                                }
                                            )
                            ...
                            try:
                                del self.activities[guild_index]["members"][
                                    memlist["index"]
                                ]
                            except IndexError:
                                pass
                            ...
                            if "group" in memlist["item"]:
                                ...
                                self.activities[guild_index]["members"].insert(
                                    memlist["index"],
                                    {"group": memlist["item"]["group"]["id"]},
                                )
                                if len(self.activities[guild_index]["members"]) > 100:
                                    self.activities[guild_index]["members"].pop(-1)
                                self.activities_changed.append(guild_id)
                                self.activities[guild_index]["last_index"] = int(
                                    memlist["index"]
                                )
                                continue
                            ...
                            if memlist["op"] == "UPDATE":
                                try:
                                    if (
                                        self.activities[guild_index]["members"][
                                            memlist["index"]
                                        ].get("id")
                                        == member_id
                                    ):
                                ...
                                self.activities[guild_index]["members"].insert(
                                    memlist["index"], ready_data
                                )
                                if (
                                    len(self.activities[guild_index]["members"]) > 100
                                ):  # lets have some limits
                                    self.activities[guild_index]["members"].pop(-1)
                            self.activities[guild_index]["last_index"] = int(
                                memlist["index"]
                            )
                    ...
                    if optext == "CHANNEL_DELETE":
                        ...
                        self.guilds[num]["channels"][num_ch] = {
                            ...
                            "permission_overwrites": new_channel[
                                "permission_overwrites"
                            ],
                            "hidden": False,
                        }
    ...
    def resume(self):
        ...
        self.ws = websocket.WebSocket()
        self.ws.connect(
            self.resume_gateway_url + "/?v=9&encoding=json&compress=zlib-stream"
        )
    ...
    def update_presence(
        self, status, custom_status=None, custom_status_emoji=None, rpc=None
    ):
