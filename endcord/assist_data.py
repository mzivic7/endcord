...
COMMAND_ASSISTS = (
    ...
    (
        "status *[type] - change your status 1/online, 2/idle, 3/dnd, 4/invisible",
        "status",
    ),
    (
        "open_link *[num] - open selected link or specify index if multiple are on same line",
        "open_link",
    ),
    ...
    (
        "channel *<#[channel_id]> - view info of selected or specified channel",
        "channel",
    ),
    (
        "summaries *<#[channel_id]> - view summaries of selected or specified channel",
        "summaries",
    ),
    ("hide *<#[channel_id]> - view info of selected or specified channel", "hide"),
    (
        "toggle_mute *<#[channel_id]> - toggle mute state of selected or specified channel",
        "toggle_mute",
    ),
    (
        "mark_as_read *<#[channel_id]> - mark channel/server/category/DM as read",
        "mark_as_read",
    ),
    ...
    (
        "goto_mention *[num] - go to channel/message mention in selected message",
        "goto_mention",
    ),
    ...
    (
        "string_select [string] - select string on interactive app message",
        "string_select",
    ),
    ...
    (
        "paste_clipboard_image - upload image from clipboard as attachment",
        "paste_clipboard_image",
    ),
    (
        "insert_timestamp YYYY-MM-DD-HH-mm / YYYY-MM-DD / HH:mm / HH:mm:SS - insert discord timestamp",
        "insert_timestamp",
    ),
    (
        "set_notifications *<#[channel_id]> ... - show and modify server/channel notification settings",
        "set_notifications",
    ),
    (
        "check_standing - check account standing, anything non-100 is concerning",
        "check_standing",
    ),
